...
NavDelegate::NavDelegate(QObject *parent) : QStyledItemDelegate(parent)
{
    ...
    //引入图形字体
    int fontId = QFontDatabase::addApplicationFont(":/Res/ttf/fontawesome-webfont.ttf");
...
NavListView::NavListView(QWidget *parent) : QListView(parent)
{
    ...
    icoStyle = NavListView::IcoStyle_Triangle;
    ...
    connect(this, SIGNAL(clicked(QModelIndex)), model, SLOT(collapse(QModelIndex)));  // Qlist点击信号发送MODEL
