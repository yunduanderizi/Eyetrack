...
////////////////////////////////////////////////////////////////////////////////////////////////////
/// @fn QLError QLCalibration_DetectFocusedTarget( QLCalibrationId calibration, int* targets,
/// int numTargets, int duration, int* focusedTarget);
///
/// @ingroup Calibration
/// @brief Identify the target that user is looking at for user driven calibration.
///
/// This function detects if user gaze is stable and then uses eye data and partial
///	calibration to identify which of the listed (painted) targets the user is looking at.
///
/// This function can only be called before a calibration has been finalized.
///
/// @param [in] calibration The ID of a calibration container whose data will be used to
///                         calculate a score. This ID is obtained by calling either the function
///                         QLCalibration_Create() or the function QLCalibration_Load().
/// @param [in] targets     The IDs of the targets that are currently painted for the user driven
///							calibration. These IDs are obtained by calling the function
///							QLCalibration_GetTargets().
/// @param [in] numTargets  The number of targets painted for the user to potentially look at.
/// @param [in] duration	The time in ms that gaze stability is measured over. This function uses
///							history data and there is not a delay associated with duration.
/// @param [out] focusedTarget      A pointer to an target ID which is the ID of the target that the
///									user is focused on.
///
/// @return The success of the function. If the return value is QL_ERROR_OK then a target was
/// successfully identified. If the return value is QL_ERROR_INTERNAL_ERROR then call the function
/// again when it is expected that the user is focusing on a target.
///
/// @example /src/QuickStart/Calibrate.cpp
////////////////////////////////////////////////////////////////////////////////////////////////////
QLError QUICK_LINK_2_CALL_CONVEN
	QLCalibration_DetectFocusedTarget(
		QLCalibrationId calibration,
		int* indices,
		int numTargets,
		int duration,
